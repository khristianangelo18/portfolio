...
// BrowserPreview Component — shows the real, live landing page inside a browser-chrome frame.
// If `image` is supplied, that screenshot is used (fastest, most reliable). Otherwise, for a real
// URL, the actual site is embedded via a scaled-down iframe so the true landing page renders here.
// Only falls back to an abstract gradient mock when there's no real link to preview at all.
// BrowserPreview Component — Retained site preview iframe/image as the core showcase element
function BrowserPreview({ title, link, image, tall = false }) {
...
// ProjectLinks — the two CTA buttons: View Project + View Source Code
function ProjectLinks({ link, github, size = 'md' }) {
  const isExternalLink = link && link !== 'javascript:void(0)' && link !== '#';
    ...
    <div className="flex flex-wrap gap-2.5" onClick={(e) => e.stopPropagation()}>
      ...
      {hasGithub && (
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className={`inline-flex items-center gap-2 ${pad} bg-zinc-900 border border-zinc-800 hover:bg-zinc-800 text-zinc-300 font-medium rounded text-xs transition-colors`}
        >
...
// ProjectModal — full case-study detail, rendered in a portal so it always covers the
// full viewport (a Reveal-animated ancestor sets a transform, which would otherwise
// trap a `fixed` element to that ancestor instead of the screen).
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6" role="dialog" aria-modal="true">
      ...
      <div className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-xl border border-zinc-800 bg-zinc-950 shadow-2xl animate-fade-in-up">
        <button
          ...
          className="absolute top-3 right-3 z-20 p-2 rounded-lg bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-white transition-colors"
        >
          <X className="w-4 h-4" />
        </button>
        ...
        <div className="p-6 sm:p-10 space-y-6">
          ...
          <div className="pt-2">
            ...
            <div className="flex flex-wrap gap-2">
              {tags.map((tag, i) => (
                <span key={i} className="px-2.5 py-1 bg-zinc-900 border border-zinc-800 rounded text-xs text-zinc-300 font-mono">
                  {tag}
                </span>
              ))}
            </div>
...
// FeaturedProjectCard — a compact, single "spotlight" card for the flagship project.
// Clicking the card (outside the CTA buttons) opens a full case-study modal.
          ...
          <div className="p-5 sm:p-6 space-y-3">
            ...
            <div className="flex flex-wrap gap-1.5 pt-1">
              {tags.map((tag, i) => (
                <span
                  key={i}
                  className="px-2 py-0.5 bg-zinc-900 border border-zinc-800 rounded text-[11px] text-zinc-400 font-mono"
                >
                  {tag}
                </span>
              ))}
        ...
        <div className="px-5 sm:px-6 pb-5 pt-3 border-t border-zinc-900 flex items-center justify-between gap-4">
          <div>
            {hasLinks ? (
              ...
              <span className="text-[11px] font-mono text-zinc-600">Mobile App</span>
            )}
          </div>
...
// ProjectCard Component — compact card used for secondary (non-featured) projects.
// Clicking the card (outside the CTA buttons) opens the same full case-study modal.
function ProjectCard(project) {
  const { title, subtitle, description, tags, link } = project;
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [mounted, setMounted] = useState(false);
